...
import { ContentType, Service, SubService, ContentPillar, Person } from "@/types/contentManagement";
...
export const useContentMetadata = () => {
  ...
  const [contentPlanners, setContentPlanners] = useState<Person[]>([]);
  const [productionTeam, setProductionTeam] = useState<Person[]>([]);
