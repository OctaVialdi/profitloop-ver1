...
export const supabase = createClient<Database>(
  ...
  SUPABASE_PUBLISHABLE_KEY,
  {
    auth: {
      ...
      debug: true // Enable debug mode to track auth-related issues
    }
  }
);
...
// Helper function to process magic link invitation
export async function processInvitation(invitationToken: string, userId: string) {
  try {
    const { data, error } = await supabase.rpc('process_magic_link_invitation', {
