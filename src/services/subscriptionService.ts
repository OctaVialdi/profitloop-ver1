...
export const checkAndUpdateTrialStatus = async (organizationId: string) => {
  try {
...
export const fixOrganizationTrialPeriod = async (organizationId: string) => {
  try {
    // Get current organization data
    const { data: org, error: orgError } = await supabase
      .from('organizations')
...
export const sendTrialReminderEmail = async (organizationId: string, email: string) => {
...
export const requestTrialExtension = async (organizationId: string, reason: string) => {
  try {
